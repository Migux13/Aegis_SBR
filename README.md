# Aegis: Single Button Rotation (v1.2.0)
...
- **Emergency buttons:** below a health share you choose, everything stops. *Divine Shield* on the DPS and Solofarming tabs, *Lay on Hands* on Tank and DPS. The tank tab deliberately does **not** offer the bubble: it drops every point of threat, so the emergency that saves the tank hands the pull to somebody who cannot survive it. Where both are set the shield goes first — five minutes is a far cheaper cooldown than an hour.
- **Hammer of Wrath leads the execute.** Out of melee reach it fires immediately; in melee it first asks whether *Judgement of the Crusader* is on the target, since that amplifies the holy damage, and takes the detour to apply it **only** when the measured time to kill exceeds the setup — Judgement's remaining cooldown, three global cooldowns and the hammer's own cast, counted honestly. The judgement lasts ten seconds, so on a normal mob that arithmetic never closes; the path is for elites by construction.
