...
| SuperCleveRoidMacros wiki — https://github.com/brues-code/SuperCleveRoidMacros/wiki | ✅ Fetches cleanly | Dependency updates (conditionals, reqs) | 2026-08-07 (**fork switched**: user runs `brues-code`, active. The `jrc13245` repo is archived; behavioural detail in `dependencies.md` was verified against THAT one and wants re-checking here. Slash-Commands page confirms `/startattack` + `/stopattack` exist; toggle-vs-start-only unstated) |
