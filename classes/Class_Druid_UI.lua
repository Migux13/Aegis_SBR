...
function M:BuildBody(ui, parent)
    ...
    L:Header("Dispel", "tree")
    self.cureRow = L:Row{ key = "useCure", label = "Cure afflictions", spell = "Remove Curse", onToggle = set("useCure") }
    self.curePctRow = L:Row{ label = "Cure first above",
        slider = { key = "curePct", min = 0, max = 100, step = 5, suffix = "%", onChange = set("curePct") } }


    L:Header("Heal priority", "tree")
    ...
    L:Header("Priority list", "tree", function()
...
function M:RefreshBody(ui, buf)
    ...
    ui:BindCheck(self.cureRow, buf.useCure, "Remove Curse")
